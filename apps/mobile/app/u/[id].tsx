...
function formatJoined(d?:string|null){ if(!d) return ''; const x=new Date(d); return `Joined ${x.toLocaleString('en-US',{month:'long',year:'numeric'})}`; }
...
export default function PublicProfile() {
  ...
  useEffect(()=>{ if(!id) return; (async()=>{ setLoading(true);
    const r=await supabase.from('profiles').select('id,username,display_name,bio,location,website,avatar_url,banner_url,verified:is_verified,created_at,avatar_version').eq('id',String(id)).maybeSingle();
    ...
    const me=meId;
    if(me){ const f=await supabase.from('follows').select('id',{count:'exact',head:true}).eq('follower_id',me).eq('followee_id',String(id)); setIsFollowing((f.count||0)>0);
           const fy=await supabase.from('follows').select('id',{count:'exact',head:true}).eq('follower_id',String(id)).eq('followee_id',me); setFollowsYou((fy.count||0)>0); }
  ...
  if(loading){
    return(<View style={{flex:1,paddingBottom:96}}><TopBar/><View style={{flex:1,alignItems:'center',justifyContent:'center'}}><ActivityIndicator/></View><NavBar/></View>);
  }

  if(!p){
    return(<View style={{flex:1,paddingBottom:96}}><TopBar/><View style={{padding:16}}><Text>Profile not found.</Text></View><NavBar/></View>);
  }
    ...
    <View style={{ flex:1, paddingBottom:96 }}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom:120 }}>
        ...
        <View style={{ paddingHorizontal:16, marginTop:-36, flexDirection:'row', justifyContent:'space-between', alignItems:'flex-end' }}>
          ...
          {!isMe ? (
            isFollowing ? null : (
              <Pressable onPress={follow} style={{ paddingHorizontal:14, paddingVertical:8, borderWidth:1, borderColor:'#ddd', borderRadius:20, backgroundColor:'#fff' }}>
        ...
        <View style={{ paddingHorizontal:16, paddingTop:8 }}>
          ...
          <View style={{ flexDirection:'row', flexWrap:'wrap', gap:14, marginTop:8, alignItems:'center' }}>
            ...
            {p.created_at ? (<View style={{ flexDirection:'row', alignItems:'center', gap:6 }}><Ionicons name="calendar-outline" size={14} color="#444"/><Text style={{ color:'#444' }}>{formatJoined(p.created_at)}</Text></View>) : null}
          </View>
