...
export default function PublicProfile() {
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      if(cancelled) return;
      if(r.data) setProfile(r.data as any); else setProfile(null);
      ...
      if(me.data?.user?.id){
        const rel=await supabase.from('follows').select('id').eq('follower_id',me.data.user.id).eq('followee_id',String(id)).maybeSingle();
  ...
  async function toggleFollow(){
    ...
    if(isFollowing){
      await supabase.from('follows').delete().eq('follower_id',me.data.user.id).eq('followee_id',String(id));
      setIsFollowing(false);
    }else{
      await supabase.from('follows').insert({ follower_id: me.data.user.id, followee_id: String(id) });
      setIsFollowing(true);
    }
  }
    ...
    <ScrollView contentContainerStyle={{ paddingBottom: 80 }}>
      ...
      <View style={{ paddingHorizontal:16, paddingTop:8 }}>
        ...
        <View style={{ flexDirection:'row', flexWrap:'wrap', gap:14, marginTop:8, alignItems:'center' }}>
          ...
          {profile.created_at ? (<View style={{ flexDirection:'row', alignItems:'center', gap:6 }}><Ionicons name="calendar-outline" size={14} color="#444" /><Text style={{ color:'#444' }}>{formatJoined(profile.created_at)}</Text></View>) : null}
        </View>
