...
type Profile = { id:string; username:string|null; display_name:string|null; avatar_url:string|null; is_verified?:boolean|null; bio?:string|null; location?:string|null; website?:string|null; created_at?:string|null };
...
export default function PublicProfile() {
  ...
  useEffect(()=>{ if(!id) return; (async()=>{ setLoading(true);
    const base=await supabase.from('profiles').select('id,username,display_name,avatar_url,is_verified,created_at').eq('id',String(id)).maybeSingle();
    if(base.data) setP(base.data as any); else { setP(null); setLoading(false); return; }
    const ext=await supabase.from('user_profiles').select('bio,location,website').eq('id',String(id)).maybeSingle();
    if(ext.data) setP(prev=>prev?{...prev,...ext.data as any}:prev);
  ...
  async function follow(){ if(!meId) return; await supabase.from('follows').insert({ follower_id: meId, followee_id: String(id) }); setIsFollowing(true); setCounts(c=>({...c,followers:c.followers+1})); }
    ...
    <View style={{ flex:1, paddingBottom:96 }}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom:120 }}>
        <View style={{ width:'100%', aspectRatio:3, backgroundColor:'#e9ecef' }}>
          <ExpoImage source={p.avatar_url?undefined:undefined} style={{ width:'100%', height:'100%' }} />
        </View>
