...
export default function PublicProfile() {
  ...
  async function toggleFollow(){
    ...
    if(amFollowing){
      await supabase.from('follows').delete().eq('follower_id', me).eq('followee_id', profile.id);
      setAmFollowing(false);
      setCounts(c=>({ ...c, followers: Math.max(0,c.followers-1) }));
    }else{
      const ins=await supabase.from('follows').insert({ follower_id: me, followee_id: profile.id }).select('id').maybeSingle();
      if(ins.error){ Alert.alert('Error', ins.error.message); return; }
      setAmFollowing(true);
      ...
      <ScrollView contentContainerStyle={{ paddingBottom:120 }}>
        ...
        <View style={{ paddingHorizontal:16, marginTop:-36, flexDirection:'row', justifyContent:'space-between', alignItems:'flex-end' }}>
          ...
          <Pressable onPress={toggleFollow} style={{ paddingHorizontal:14, paddingVertical:8, borderWidth:1, borderColor:'#ddd', borderRadius:20, backgroundColor:'#fff' }}>
            <Text style={{ fontWeight:'600' }}>{amFollowing ? 'Following' : 'Follow'}</Text>
          </Pressable>
        </View>
