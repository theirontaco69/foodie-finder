...
function Row({ icon, label, onPress }:{ icon:string; label:string; onPress:()=>void }){
  return(<Pressable onPress={onPress} style={{ flexDirection:'row', alignItems:'center', gap:14, paddingVertical:12 }}><Ionicons name={icon as any} size={22} color="#111" /><Text style={{ fontSize:16 }}>{label}</Text></Pressable>);
}
...
export default function SideMenu({ open, onClose }:{ open:boolean; onClose:()=>void }){
  ...
  async function syncSession(){ const s=await supabase.auth.getSession(); const id=s.data?.session?.user?.id??null; setMeId(id); return id; }
  useEffect(()=>{ syncSession(); },[]);
  useEffect(()=>{ const sub=supabase.auth.onAuthStateChange(async()=>{ await syncSession(); }); return ()=>sub.data.subscription.unsubscribe(); },[]);
  useEffect(()=>{ if(open) syncSession(); },[open]);
  ...
  const url=resolveAvatarPublicUrl(supabase, p?.avatar_url??null, { userId:p?.id??undefined, version:p?.avatar_version??undefined }) ?? (p?.display_name||p?.username?fallbackAvatar(p?.display_name||p?.username):null);

  return(
    <View style={{ flex:1, backgroundColor:'#fff', paddingTop:60, paddingHorizontal:16 }}>
      {loading?(<View style={{ alignItems:'center', justifyContent:'center', padding:20 }}><ActivityIndicator/></View>):
        meId&&p?(
          <View style={{ gap:16 }}>
            <View style={{ flexDirection:'row', alignItems:'center', gap:12 }}>
              <View style={{ width:56, height:56, borderRadius:28, overflow:'hidden', backgroundColor:'#eee' }}>
                {url?<ExpoImage source={{ uri:url }} style={{ width:'100%', height:'100%' }} contentFit="cover" />:null}
              </View>
              <View>
